...
// вспомогательные макросы поиска текстуры спектра
...
// метаданные текстуры черного тела
...
// система координат текстуры фона
mat3 BG_COORDS = ROT_Y(45.0 * DEG_TO_RAD);

// система координат текстуры планеты
...
// функция для вычисления пересечения планеты с Чёрной дырой
vec4 planet_intersection(vec3 old_pos, vec3 ray, float t, float dt,
    ...
    // преобразование в систему координат движущейся планеты
    ray = ray - planet_vel;
    {{/light_travel_time}}
    {{^light_travel_time}}
...
void main() {
    ...
    // горизонт событий находится на уровне u = 1
    if (u < 1.0) {
